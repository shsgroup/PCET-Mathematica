...
NMorseOverlap[DE1_,Beta1_,DE2_,Beta2_,nu1_,nu2_,d_,OptionsPattern[{M->MassH}]]:=Module[{Mass,mu,DE1A,DE2A,X,integrand},
	...
	NIntegrate[integrand[X],{X,-\[Infinity],\[Infinity]},Method->{"DoubleExponential","SymbolicProcessing"->0},Compiled->False,MinRecursion->6,MaxRecursion->20]
];
...
command=BsplineCommand<>" "<>file<>" "<>ToString[nG]<>" "<>massString<>" "<>ToString[nS];
error=Run[command];
...
HarmonicFranckCondonAveraged[mu_,nu_,f1_,f2_,d_,T_,MR_,FR_,OptionsPattern[{M->MassH,Distribution->"Classical"}]]:=
Module[{m,o1,o2,distr,integrand},
   m=OptionValue[M];
...
Module[{m,sw,PR,s,distr,integrand},
   ...
   PR[R_]:=Switch[distr,
                        "Classical",PClassical[T,d,MR,FR,R],
                        "Quantal",  PQuantal[T,d,MR,FR,R],
                        _,          PClassical[T,d,MR,FR,R]];
   s[R_]:=Switch[sw,
...
Module[{E0,Emu,Z0,Pmu,mu,nu,Ns},
	...
	Do[Emu[mu]=kcal2au*fghList[[1,1,mu+1]],{mu,0,Ns-1}];
	E0=Emu[0];
	Z0=Sum[Exp[-(Emu[mu]-E0)/(kb*T)],{mu,0,Ns-1}];
	Do[Pmu[mu]=Exp[-(Emu[mu]-E0)/(kb*T)]/Z0,{mu,0,mumax}];
...
Module[{distr,headings,dg0,la,totalrate,s,s2,alpha,dg,Ea,w,Emu,Enu,Pmu,Z,kmunu,fc,info,Mass},
	distr=OptionValue[Distribution];
	Mass=OptionValue[M];
	headings={"\[Mu]","\[Nu]",strpmu,strdgmunu,strdgddmunu,"<"<>strs2munu<>">","exp(-\[Beta]"<>strdgddmunu<>")","%"};
